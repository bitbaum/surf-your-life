...
vi.mock("@/lib/db", () => ({ db: {} }))
vi.mock("@/lib/domain/embeddings", () => ({ semanticCheckInSearch: vi.fn().mockResolvedValue(null) }))

import { ruleBasedResponse, type CheckInRow, type MedicationRow, type AssessmentRow } from "@/lib/domain/ai-chat"
...
describe("ruleBasedResponse — PEM", () => {
  ...
  it("reports PEM count when episodes exist", () => {
    ...
    const reply = ruleBasedResponse("Tell me about my crash patterns", rows, NO_MEDS, NO_ASSESSMENT)
  ...
  it("triggers on 'exertional' keyword", () => {
    const rows = [row()]
    const reply = ruleBasedResponse("post-exertional malaise question", rows, NO_MEDS, NO_ASSESSMENT)
...
describe("ruleBasedResponse — progress", () => {
  ...
  it("detects upward energy trend", () => {
    ...
    const older = [row({ energyLevel: 4, createdAt: new Date(Date.now() - SEVEN_DAYS_MS - DAY_MS) })]
  ...
  it("detects downward energy trend", () => {
    ...
    const older = [row({ energyLevel: 8, createdAt: new Date(Date.now() - SEVEN_DAYS_MS - DAY_MS) })]
    const reply = ruleBasedResponse("getting worse?", [...recent, ...older], NO_MEDS, NO_ASSESSMENT)
  ...
  it("reports stable when energy difference < 0.5", () => {
    ...
    const older = [row({ energyLevel: 5, createdAt: new Date(Date.now() - SEVEN_DAYS_MS - DAY_MS) })]
...
describe("ruleBasedResponse — orthostatic", () => {
  ...
  it("reports orthostatic count and flags for discussion", () => {
    const rows = [row({ orthostaticSymptoms: true }), row({ orthostaticSymptoms: true }), row({ orthostaticSymptoms: false })]
