...
describe("createTechniqueSchema", () => {
  ...
  it("accepts technique with only required fields", () => {
    expect(
      createTechniqueSchema.safeParse({ name: "Box Breathing", category: "breathwork" }).success,
    ).toBe(true);
  });
  ...
  it(`rejects name exceeding ${TECHNIQUE_NAME_MAX} characters`, () => {
    expect(
      createTechniqueSchema.safeParse({ ...valid, name: "x".repeat(TECHNIQUE_NAME_MAX + 1) })
        .success,
    ).toBe(false);
  });
  ...
  it(`accepts name exactly ${TECHNIQUE_NAME_MAX} characters`, () => {
    expect(
      createTechniqueSchema.safeParse({ ...valid, name: "x".repeat(TECHNIQUE_NAME_MAX) }).success,
    ).toBe(true);
  });

  it(`rejects description exceeding ${FIELD_MAX_NOTES} characters`, () => {
    expect(
      createTechniqueSchema.safeParse({ ...valid, description: "x".repeat(FIELD_MAX_NOTES + 1) })
        .success,
    ).toBe(false);
  });
  ...
  it(`rejects instructions exceeding ${FIELD_MAX_LONG} characters`, () => {
    expect(
      createTechniqueSchema.safeParse({ ...valid, instructions: "x".repeat(FIELD_MAX_LONG + 1) })
        .success,
    ).toBe(false);
  });
  ...
  it(`rejects durationMinutes below ${TECHNIQUE_DURATION_MINUTES.min}`, () => {
    expect(
      createTechniqueSchema.safeParse({
        ...valid,
        durationMinutes: TECHNIQUE_DURATION_MINUTES.min - 1,
      }).success,
    ).toBe(false);
  });
  ...
  it(`rejects durationMinutes above ${TECHNIQUE_DURATION_MINUTES.max}`, () => {
    expect(
      createTechniqueSchema.safeParse({
        ...valid,
        durationMinutes: TECHNIQUE_DURATION_MINUTES.max + 1,
      }).success,
    ).toBe(false);
  });
  ...
  it("accepts valid URL for resourceUrl", () => {
    expect(
      createTechniqueSchema.safeParse({ ...valid, resourceUrl: "https://example.com/guide" })
        .success,
    ).toBe(true);
  });

  it("rejects non-empty non-URL string for resourceUrl", () => {
    expect(createTechniqueSchema.safeParse({ ...valid, resourceUrl: "not-a-url" }).success).toBe(
      false,
    );
  });
});
...
describe("createAssignmentSchema", () => {
  ...
  it("accepts optional endDate", () => {
    expect(createAssignmentSchema.safeParse({ ...valid, endDate: "2024-12-31" }).success).toBe(
      true,
    );
  });

  it("rejects non-UUID techniqueId", () => {
    expect(createAssignmentSchema.safeParse({ ...valid, techniqueId: "not-uuid" }).success).toBe(
      false,
    );
  });

  it("rejects non-UUID clientId", () => {
    expect(createAssignmentSchema.safeParse({ ...valid, clientId: "not-uuid" }).success).toBe(
      false,
    );
  });
  ...
  it(`rejects frequencyPerDay above ${TECHNIQUE_DAILY_FREQUENCY_MAX}`, () => {
    expect(
      createAssignmentSchema.safeParse({
        ...valid,
        frequencyPerDay: TECHNIQUE_DAILY_FREQUENCY_MAX + 1,
      }).success,
    ).toBe(false);
  });

  it("rejects non-integer frequencyPerDay", () => {
    expect(createAssignmentSchema.safeParse({ ...valid, frequencyPerDay: 1.5 }).success).toBe(
      false,
    );
  });

  it("rejects safetyCapMultiplier below 100 (below 1× daily target)", () => {
    expect(createAssignmentSchema.safeParse({ ...valid, safetyCapMultiplier: 99 }).success).toBe(
      false,
    );
  });

  it("rejects safetyCapMultiplier above 300 (above 3× daily target)", () => {
    expect(createAssignmentSchema.safeParse({ ...valid, safetyCapMultiplier: 301 }).success).toBe(
      false,
    );
  });
  ...
  it("rejects malformed startDate", () => {
    expect(createAssignmentSchema.safeParse({ ...valid, startDate: "01/09/2024" }).success).toBe(
      false,
    );
    expect(createAssignmentSchema.safeParse({ ...valid, startDate: "2024-9-1" }).success).toBe(
      false,
    );
  });
  ...
  it(`rejects notes exceeding ${FIELD_MAX_NOTES} characters`, () => {
    expect(
      createAssignmentSchema.safeParse({ ...valid, notes: "x".repeat(FIELD_MAX_NOTES + 1) })
        .success,
    ).toBe(false);
  });
});
...
describe("logTechniqueSchema", () => {
  ...
  it("rejects non-UUID assignmentId", () => {
    expect(logTechniqueSchema.safeParse({ ...valid, assignmentId: "not-uuid" }).success).toBe(
      false,
    );
  });
  ...
  it(`rejects notes exceeding ${FIELD_MAX_NOTES} characters`, () => {
    expect(
      logTechniqueSchema.safeParse({ ...valid, notes: "x".repeat(FIELD_MAX_NOTES + 1) }).success,
    ).toBe(false);
  });

  it(`accepts notes exactly ${FIELD_MAX_NOTES} characters`, () => {
    expect(
      logTechniqueSchema.safeParse({ ...valid, notes: "x".repeat(FIELD_MAX_NOTES) }).success,
    ).toBe(true);
  });
