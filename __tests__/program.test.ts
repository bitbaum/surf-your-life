...
describe("phaseSchema", () => {
  ...
  it(`rejects week above ${PROGRAM_DURATION_WEEKS_MAX}`, () => {
    expect(phaseSchema.safeParse({ ...valid, week: PROGRAM_DURATION_WEEKS_MAX + 1 }).success).toBe(
      false,
    );
  });
  ...
  it(`accepts boundary weeks (1 and ${PROGRAM_DURATION_WEEKS_MAX})`, () => {
    ...
    expect(phaseSchema.safeParse({ ...valid, week: PROGRAM_DURATION_WEEKS_MAX }).success).toBe(
      true,
    );
  });
  ...
  it(`rejects title exceeding ${FIELD_MAX_TITLE} characters`, () => {
    expect(
      phaseSchema.safeParse({ ...valid, title: "x".repeat(FIELD_MAX_TITLE + 1) }).success,
    ).toBe(false);
  });

  it(`accepts title exactly ${FIELD_MAX_TITLE} characters`, () => {
    expect(phaseSchema.safeParse({ ...valid, title: "x".repeat(FIELD_MAX_TITLE) }).success).toBe(
      true,
    );
  });
  ...
  it(`rejects guidance exceeding ${FIELD_MAX_LONG} characters`, () => {
    expect(
      phaseSchema.safeParse({ ...valid, guidance: "x".repeat(FIELD_MAX_LONG + 1) }).success,
    ).toBe(false);
  });

  it(`accepts guidance exactly ${FIELD_MAX_LONG} characters`, () => {
    expect(phaseSchema.safeParse({ ...valid, guidance: "x".repeat(FIELD_MAX_LONG) }).success).toBe(
      true,
    );
  });
});
...
describe("createProgramSchema", () => {
  ...
  it(`rejects title exceeding ${FIELD_MAX_TITLE} characters`, () => {
    expect(
      createProgramSchema.safeParse({ ...valid, title: "x".repeat(FIELD_MAX_TITLE + 1) }).success,
    ).toBe(false);
  });

  it(`rejects description exceeding ${FIELD_MAX_LONG} characters`, () => {
    expect(
      createProgramSchema.safeParse({ ...valid, description: "x".repeat(FIELD_MAX_LONG + 1) })
        .success,
    ).toBe(false);
  });
  ...
  it(`accepts description exactly ${FIELD_MAX_LONG} characters`, () => {
    expect(
      createProgramSchema.safeParse({ ...valid, description: "x".repeat(FIELD_MAX_LONG) }).success,
    ).toBe(true);
  });
  ...
  it(`rejects durationWeeks above ${PROGRAM_DURATION_WEEKS_MAX}`, () => {
    expect(
      createProgramSchema.safeParse({ ...valid, durationWeeks: PROGRAM_DURATION_WEEKS_MAX + 1 })
        .success,
    ).toBe(false);
  });
  ...
  it(`accepts durationWeeks boundary values (1 and ${PROGRAM_DURATION_WEEKS_MAX})`, () => {
    ...
    expect(
      createProgramSchema.safeParse({ ...valid, durationWeeks: PROGRAM_DURATION_WEEKS_MAX })
        .success,
    ).toBe(true);
  });
  ...
  it("accepts all valid targetConcern values", () => {
    for (const concern of MAIN_CONCERNS) {
      expect(createProgramSchema.safeParse({ ...valid, targetConcern: concern }).success).toBe(
        true,
      );
    }
  });
  ...
  it("rejects invalid targetConcern", () => {
    expect(createProgramSchema.safeParse({ ...valid, targetConcern: "stress" }).success).toBe(
      false,
    );
  });
...
describe("enrollClientSchema", () => {
  ...
  it(`rejects notes exceeding ${FIELD_MAX_LONG} characters`, () => {
    expect(
      enrollClientSchema.safeParse({ ...valid, notes: "x".repeat(FIELD_MAX_LONG + 1) }).success,
    ).toBe(false);
  });

  it(`accepts notes exactly ${FIELD_MAX_LONG} characters`, () => {
    expect(
      enrollClientSchema.safeParse({ ...valid, notes: "x".repeat(FIELD_MAX_LONG) }).success,
    ).toBe(true);
  });
});
...
describe("updateEnrollmentSchema", () => {
  ...
  it("accepts optional notes with status", () => {
    expect(
      updateEnrollmentSchema.safeParse({ status: "completed", notes: "Great progress." }).success,
    ).toBe(true);
  });

  it(`rejects notes exceeding ${FIELD_MAX_LONG} characters`, () => {
    expect(
      updateEnrollmentSchema.safeParse({ status: "active", notes: "x".repeat(FIELD_MAX_LONG + 1) })
        .success,
    ).toBe(false);
  });
  ...
  it(`accepts notes exactly ${FIELD_MAX_LONG} characters`, () => {
    expect(
      updateEnrollmentSchema.safeParse({ status: "active", notes: "x".repeat(FIELD_MAX_LONG) })
        .success,
    ).toBe(true);
  });
});
