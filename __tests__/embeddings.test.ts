...
const CHECK_IN = { mood: "low", energyLevel: 3, sleepHours: 6, sleepQuality: 2, activityLevel: "rest", pemFlag: false, pemSeverity: null, orthostaticSymptoms: false, symptomFatigue: 7, symptomBrainFog: 5, symptomPain: 2, stressLevel: 6, journalEntry: "rough day", wins: null, challenges: null, notes: null }
...
beforeEach(() => {
  ...
  findFirstDocument.mockReset().mockResolvedValue({ title: "Lab report", content: "ferritin 12", type: "lab" })
...
describe("embedCheckIn reports whether a row was written", () => {
  ...
  it("returns false WITHOUT writing when OpenAI rejects the request", async () => {
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue({
      ok: false, status: 401, statusText: "Unauthorized", text: async () => "invalid_api_key",
    }))
