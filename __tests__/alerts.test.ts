...
vi.mock("@/lib/email", () => ({
  sendEmail: vi.fn(),
  sendEmailSafe: vi.fn().mockResolvedValue(true),
}));
vi.mock("@/lib/email/templates", () => ({
  practitionerAlertEmail: vi.fn(),
  missedCheckInDigestEmail: vi.fn(),
  techniqueAdherenceDigestEmail: vi.fn(),
}));

import {
  evaluateAlertRules,
  checkTechniqueAdherenceDecline,
  type CheckInDataForAlerts,
...
describe("evaluateAlertRules — Rule 2: fatigue spike", () => {
  it(`fires when fatigue = ${ALERT_FATIGUE_SPIKE_THRESHOLD} (threshold)`, () => {
    expect(
      evaluateAlertRules(
        [makeCheckIn({ symptomFatigue: ALERT_FATIGUE_SPIKE_THRESHOLD })],
        NOW,
  ...
  it("fires for fatigue above threshold", () => {
    expect(
      evaluateAlertRules(
        [makeCheckIn({ symptomFatigue: ALERT_FATIGUE_SPIKE_THRESHOLD + 1 })],
        NOW,
      ).some((a) => a.type === "fatigue_spike"),
    ).toBe(true);
  });
  ...
  it("does not fire when fatigue is null", () => {
    expect(
      evaluateAlertRules([makeCheckIn({ symptomFatigue: null })], NOW).some(
        (a) => a.type === "fatigue_spike",
      ),
    ).toBe(false);
  });

  it("sets severity 'high' for fatigue ≥ 9", () => {
    const alert = evaluateAlertRules([makeCheckIn({ symptomFatigue: 9 })], NOW).find(
      (a) => a.type === "fatigue_spike",
    );
    expect(alert?.severity).toBe("high");
  });
  ...
  it("sets severity 'medium' for fatigue = 8", () => {
    const alert = evaluateAlertRules([makeCheckIn({ symptomFatigue: 8 })], NOW).find(
      (a) => a.type === "fatigue_spike",
    );
    expect(alert?.severity).toBe("medium");
  });
...
describe("evaluateAlertRules — Rule 3: stress spike", () => {
  it(`fires when stress = ${ALERT_STRESS_SPIKE_THRESHOLD} (threshold)`, () => {
    expect(
      evaluateAlertRules([makeCheckIn({ stressLevel: ALERT_STRESS_SPIKE_THRESHOLD })], NOW).some(
        (a) => a.type === "stress_spike",
      ),
    ).toBe(true);
  });
  ...
  it("does not fire when stress is below threshold", () => {
    expect(
      evaluateAlertRules(
        [makeCheckIn({ stressLevel: ALERT_STRESS_SPIKE_THRESHOLD - 1 })],
        NOW,
  ...
  it("does not fire when stress is null", () => {
    expect(
      evaluateAlertRules([makeCheckIn({ stressLevel: null })], NOW).some(
        (a) => a.type === "stress_spike",
      ),
    ).toBe(false);
  });
  ...
  it("sets severity 'high' for stress ≥ 9", () => {
    const alert = evaluateAlertRules([makeCheckIn({ stressLevel: 9 })], NOW).find(
      (a) => a.type === "stress_spike",
    );
    expect(alert?.severity).toBe("high");
  });
  ...
  it("sets severity 'medium' for stress = 8", () => {
    const alert = evaluateAlertRules([makeCheckIn({ stressLevel: 8 })], NOW).find(
      (a) => a.type === "stress_spike",
    );
    expect(alert?.severity).toBe("medium");
  });
...
describe("evaluateAlertRules — Rule 4: PEM cluster", () => {
  ...
  it("does not fire when PEM flags are outside the 7-day window", () => {
    const recent = [
      ...
      makeCheckIn({
        pemFlag: true,
        createdAt: new Date(NOW.getTime() - SEVEN_DAYS_MS - 2 * DAY_MS),
      }),
    ];
...
describe("evaluateAlertRules — Rule 6: orthostatic cluster", () => {
  it(`fires when ≥${ALERT_ORTHOSTATIC_CLUSTER_COUNT} orthostatic episodes within 7 days`, () => {
    ...
    expect(evaluateAlertRules(recent, NOW).some((a) => a.type === "orthostatic_intolerance")).toBe(
      true,
    );
  });
  ...
  it("does not fire when episodes are outside the 7-day window", () => {
    const recent = [
      makeCheckIn({
        ...
        createdAt: new Date(NOW.getTime() - SEVEN_DAYS_MS - DAY_MS),
      }),
      makeCheckIn({
        orthostaticSymptoms: true,
        createdAt: new Date(NOW.getTime() - SEVEN_DAYS_MS - 2 * DAY_MS),
...
function makeLogs(
  dates: string[],
): Array<{ assignmentId: string; date: string; completedReps: number; userId: string }> {
...
describe("checkTechniqueAdherenceDecline — baseline", () => {
  ...
  it("returns empty when prior7avg is 0 (no baseline)", () => {
    ...
    const results = checkTechniqueAdherenceDecline(
      [BASE_ASSIGN],
      makeLogs(RECENT7_DATES),
      TECH_TODAY,
    );
