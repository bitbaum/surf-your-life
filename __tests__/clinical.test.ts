...
describe("functionalAssessmentSchema", () => {
  ...
  it(`rejects overallCapacity below min (${CAPACITY_SCALE.min})`, () => {
    const result = functionalAssessmentSchema.safeParse({
      overallCapacity: CAPACITY_SCALE.min - 1,
    });
  ...
  it(`rejects overallCapacity above max (${CAPACITY_SCALE.max})`, () => {
    const result = functionalAssessmentSchema.safeParse({
      overallCapacity: CAPACITY_SCALE.max + 1,
    });
    expect(result.success).toBe(false);
  });
  ...
  it(`accepts boundary values (${CAPACITY_SCALE.min} and ${CAPACITY_SCALE.max})`, () => {
    expect(
      functionalAssessmentSchema.safeParse({ overallCapacity: CAPACITY_SCALE.min }).success,
    ).toBe(true);
    expect(
      functionalAssessmentSchema.safeParse({ overallCapacity: CAPACITY_SCALE.max }).success,
    ).toBe(true);
  });
  ...
  it("rejects optional capacity fields outside scale bounds", () => {
    expect(
      functionalAssessmentSchema.safeParse({
        overallCapacity: 5,
        cognitiveCapacity: CAPACITY_SCALE.min - 1,
      }).success,
    ).toBe(false);
    expect(
      functionalAssessmentSchema.safeParse({
        overallCapacity: 5,
        physicalCapacity: CAPACITY_SCALE.max + 1,
      }).success,
    ).toBe(false);
  });
...
describe("medicationEntrySchema", () => {
  ...
  it(`rejects medicationName exceeding ${FIELD_MAX_TITLE} characters`, () => {
    const result = medicationEntrySchema.safeParse({
      medicationName: "x".repeat(FIELD_MAX_TITLE + 1),
    });
    expect(result.success).toBe(false);
  });
