...
describe("computeReturnNudge", () => {
  ...
  const dayAt = (offset: number, hour = 9) => new Date(2026, 3, 25 + offset, hour, 0, 0);
...
describe("computeWeekDelta", () => {
  const now = new Date(2026, 3, 25, 12, 0, 0);
  const dayAt = (offset: number, hour = 9): Date => new Date(2026, 3, 25 + offset, hour, 0, 0);
...
describe("computeInsight", () => {
  ...
  it("fires PEM improving when last week had a cluster and this week is clearly down", () => {
    ...
    expect(computeInsight([5, 5, 5], 5, weekDeltaWithPem(0, 2))).toEqual({
      key: "insightPemImproving",
    });
    // 3 last week, 1 this week (delta=-2)
    expect(computeInsight([5, 5, 5], 5, weekDeltaWithPem(1, 3))).toEqual({
      key: "insightPemImproving",
    });
  });
