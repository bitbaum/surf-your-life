...
describe("checkInSchema", () => {
  ...
  it(`accepts energyLevel at boundaries [${ENERGY_SCALE.min}, ${ENERGY_SCALE.max}]`, () => {
    expect(checkInSchema.safeParse({ ...VALID_BASE, energyLevel: ENERGY_SCALE.min }).success).toBe(
      true,
    );
    expect(checkInSchema.safeParse({ ...VALID_BASE, energyLevel: ENERGY_SCALE.max }).success).toBe(
      true,
    );
  });
  ...
  it(`rejects symptomFatigue outside [${SYMPTOM_SCALE.min}, ${SYMPTOM_SCALE.max}]`, () => {
    expect(
      checkInSchema.safeParse({ ...VALID_BASE, symptomFatigue: SYMPTOM_SCALE.min - 1 }).success,
    ).toBe(false);
    expect(
      checkInSchema.safeParse({ ...VALID_BASE, symptomFatigue: SYMPTOM_SCALE.max + 1 }).success,
    ).toBe(false);
  });

  it(`rejects pemSeverity outside [${PEM_SEVERITY_SCALE.min}, ${PEM_SEVERITY_SCALE.max}]`, () => {
    expect(
      checkInSchema.safeParse({ ...VALID_BASE, pemSeverity: PEM_SEVERITY_SCALE.min - 1 }).success,
    ).toBe(false);
    expect(
      checkInSchema.safeParse({ ...VALID_BASE, pemSeverity: PEM_SEVERITY_SCALE.max + 1 }).success,
    ).toBe(false);
  });
  ...
  it(`rejects sleepQuality outside [${SLEEP_QUALITY_SCALE.min}, ${SLEEP_QUALITY_SCALE.max}]`, () => {
    expect(
      checkInSchema.safeParse({ ...VALID_BASE, sleepQuality: SLEEP_QUALITY_SCALE.min - 1 }).success,
    ).toBe(false);
    expect(
      checkInSchema.safeParse({ ...VALID_BASE, sleepQuality: SLEEP_QUALITY_SCALE.max + 1 }).success,
    ).toBe(false);
  });
  ...
  it(`rejects notes longer than ${FIELD_MAX_LONG} characters`, () => {
    const result = checkInSchema.safeParse({
      ...VALID_BASE,
      notes: "a".repeat(FIELD_MAX_LONG + 1),
    });
    expect(result.success).toBe(false);
  });
  ...
  it(`rejects journalEntry longer than ${FIELD_MAX_JOURNAL} characters`, () => {
    const result = checkInSchema.safeParse({
      ...VALID_BASE,
      journalEntry: "a".repeat(FIELD_MAX_JOURNAL + 1),
    });
    expect(result.success).toBe(false);
  });
});
...
describe("practitionerNoteSchema", () => {
  it("accepts a valid note", () => {
    const result = practitionerNoteSchema.safeParse({
      note: "Client showed improvement this week.",
    });
    expect(result.success).toBe(true);
  });
  ...
  it(`rejects note longer than ${PRACTITIONER_NOTE_MAX_LENGTH} characters`, () => {
    const result = practitionerNoteSchema.safeParse({
      note: "a".repeat(PRACTITIONER_NOTE_MAX_LENGTH + 1),
    });
    expect(result.success).toBe(false);
  });
  ...
  it(`accepts note at the max length (${PRACTITIONER_NOTE_MAX_LENGTH} chars)`, () => {
    const result = practitionerNoteSchema.safeParse({
      note: "a".repeat(PRACTITIONER_NOTE_MAX_LENGTH),
    });
    expect(result.success).toBe(true);
  });
