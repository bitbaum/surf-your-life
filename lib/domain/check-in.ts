...
export function summariseCheckIns(rows: CheckInSummaryRow[]) {
  ...
  const avgMood =
    MOODS.find((m) => MOOD_SCORE[m.value] === avgMoodScore)?.label.toLowerCase() ?? "neutral";
...
export function computeReturnNudge(
  ...
  if (days === 1) {
    return streak >= 2 ? { kind: "streak-keep", streak, days: 1 } : { kind: "yesterday", days: 1 };
  }
...
function aggregateWeek(rows: WeekDeltaRow[]): WeekAggregate {
  ...
  return {
    avgEnergy:
      energyVals.length > 0 ? energyVals.reduce((s, v) => s + v, 0) / energyVals.length : null,
    pemCount: rows.filter((r) => r.pemFlag === true).length,
    avgStress:
      stressVals.length > 0 ? stressVals.reduce((s, v) => s + v, 0) / stressVals.length : null,
    count: rows.length,
  };
}
 ...
 */
export function computeWeekDelta(rows: WeekDeltaRow[], now: Date = new Date()): WeekDelta {
  ...
  return {
    ...
    prior,
    energyDelta:
      window.avgEnergy != null && prior.avgEnergy != null
        ? round1(window.avgEnergy - prior.avgEnergy)
        : null,
    pemDelta: window.pemCount - prior.pemCount,
    stressDelta:
      window.avgStress != null && prior.avgStress != null
...
export function computeProgramProgress(enrollment: {
  ...
  const currentPhase =
    phases?.filter((p) => p.week <= currentWeek)?.sort((a, b) => b.week - a.week)[0] ?? null;
...
export type MilestoneHit = { type: "checkins"; n: number } | { type: "streak"; n: number };

export function detectMilestone(totalCheckIns: number, streak: number): MilestoneHit | null {
...
export function computeInsight(
  ...
  if (weekDelta) {
    ...
    if (weekDelta.window.pemCount >= ALERT_PEM_CLUSTER_COUNT && weekDelta.pemDelta >= 0) {
    ...
    if (weekDelta.prior.pemCount >= ALERT_PEM_CLUSTER_COUNT && weekDelta.pemDelta <= -1) {
