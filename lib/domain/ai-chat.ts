...
async function buildContext(userId: string, query?: string) {
  ...
  const [recentFallback, semanticResults, medications, latestAssessment, activeAssignments, recentTechniqueLogs, activeEnrollment] = await Promise.all([
  ...
  return { recent, medications, latestAssessment, activeAssignments, recentTechniqueLogs, activeEnrollment }
}
...
function buildTechniqueLines(
  ...
  const trend = computeDailyAdherenceTrend(
    assignments.map((a) => ({ id: a.id, clientId: "", frequencyPerDay: a.frequencyPerDay, startDate: a.startDate, endDate: a.endDate })),
    logs.map((l) => ({ assignmentId: l.assignmentId, date: l.date, completedReps: l.completedReps, userId: "" })),
    today,
    TECHNIQUE_LOG_WINDOW_DAYS
  )
  const daysLogged = trend.filter((d) => d.pct > 0).length
  const avgPct = trend.length > 0 ? Math.round(trend.reduce((s, d) => s + d.pct, 0) / trend.length) : 0
  const lines = assignments.map((a) => {
    const aLogs = logs.filter((l) => l.assignmentId === a.id)
    const daysCompleted = [...new Set(aLogs.filter((l) => l.completedReps >= a.frequencyPerDay).map((l) => l.date))].length
    const pct = TECHNIQUE_LOG_WINDOW_DAYS > 0 ? Math.round((daysCompleted / TECHNIQUE_LOG_WINDOW_DAYS) * 100) : 0
...
export function ruleBasedResponse(
  ...
  if (q.includes("technique") || q.includes("exercise") || q.includes("practice") || q.includes("habit") || q.includes("adherence") || q.includes("breathing") || q.includes("meditation") || q.includes("pacing")) {
  ...
  if (q.includes("program") || q.includes("plan") || q.includes("week") || q.includes("phase") || q.includes("schedule") || q.includes("what should i")) {
...
async function callAnthropicApi(
  ...
  const { recent, medications, latestAssessment, activeAssignments, recentTechniqueLogs, activeEnrollment } = context
  const today = localDateString(new Date())
...
export async function generateAiReply(
  ...
  return ruleBasedResponse(userMessage, recent, medications, latestAssessment, activeAssignments, recentTechniqueLogs, activeEnrollment)
}
