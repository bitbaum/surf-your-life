...
export const MOOD_EMOJI = Object.fromEntries(MOODS.map((m) => [m.value, m.emoji])) as Record<
  string,
  string
>;
export const MOOD_LABEL = Object.fromEntries(MOODS.map((m) => [m.value, m.label])) as Record<
  string,
  string
>;
...
export const ACTIVITY_LEVEL_MAP = Object.fromEntries(
  ACTIVITY_LEVELS.map((a) => [a.value, a]),
) as Record<string, (typeof ACTIVITY_LEVELS)[number]>;
...
// Loki feedback widget — only mounted on public marketing/auth pages, never on the
// authenticated client portal or admin area, which render clinical health data.
export const FEEDBACK_WIDGET_SRC = "https://loki.orangecat.ch/widget.js";
