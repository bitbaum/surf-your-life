...
export async function requireAuth(): Promise<AuthOk | AuthFail> {
  ...
  if (!session?.user?.id)
    return {
      ok: false,
      response: NextResponse.json({ success: false, error: API_ERR_UNAUTHORIZED }, { status: 401 }),
    };
...
export async function requireStaffAuth(): Promise<AuthOk | AuthFail> {
  ...
  if (!isStaff(session.user.role))
    return {
      ok: false,
      response: NextResponse.json({ success: false, error: API_ERR_FORBIDDEN }, { status: 403 }),
    };
  return { ok: true, session };
}
