    ...
    <div className="max-w-3xl mx-auto">
      ...
      <form method="get" action="/check-ins" className="mb-6">
        <div className="flex gap-2">
          ...
          />
          {q && (
            <Link href="/check-ins" className="text-sm text-slate-500 hover:text-slate-700 px-2 py-1.5 self-center">
              {t("clearSearch")}
            </Link>
          )}
        </div>
      ...
      {items.length === 0 ? (
        <div className="py-16">
          {q ? (
            <EmptyState
              message={t("noResults")}
              action={<Link href="/check-ins"><Button variant="outline">{t("clearSearch")}</Button></Link>}
            />
          ) : (
            <EmptyState
