        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <span className="text-sm font-semibold text-teal-900">{t("practitionerNoteTitle")}</span>
              {latest.practitionerNoteAt && (
                <span className="text-xs text-teal-600">{formatDate(latest.practitionerNoteAt)}</span>
              )}
            </div>
            <p className="text-sm text-teal-800 leading-relaxed">{latest.practitionerNote}</p>
            <Link href="/check-ins" className="text-xs text-teal-600 hover:underline mt-2 inline-block">
