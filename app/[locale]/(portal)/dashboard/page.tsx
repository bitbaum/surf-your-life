...
export default async function DashboardPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const [
    ...
    db.query.checkIns.findMany({
      where: and(eq(checkIns.userId, session.user.id), gte(checkIns.createdAt, thirtyDaysAgo)),
    ...
    db.query.programEnrollments.findFirst({
      ...
      with: {
        program: { columns: { id: true, title: true, durationWeeks: true, phaseConfig: true } },
      },
  ...
  const zurichHour = parseInt(
    new Intl.DateTimeFormat("en", { timeZone: CLINIC_TZ, hour: "numeric", hour12: false }).format(
      new Date(),
    ),
    10,
  );
  const greetingKey =
    zurichHour < 12 ? "greetingMorning" : zurichHour < 17 ? "greetingAfternoon" : "greetingEvening";

  const isOnboarded = ONBOARDING_REQUIRED_FIELDS.every((f) => profile?.[f as keyof typeof profile]);
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <DashboardAIInsightCard
        insight={latestAIInsight?.aiInsight ?? null}
        generatedAt={latestAIInsight?.createdAt ?? null}
      />
