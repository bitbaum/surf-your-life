...
export async function DashboardTechniquesCard({ userId }: { userId: string }) {
  ...
  const doneToday = assignments.filter((a) => (repsTodayById[a.id] ?? 0) >= a.frequencyPerDay).length
      ...
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-3 flex-shrink-0">
            ...
            <Link href="/techniques" className="text-xs text-teal-600 hover:text-teal-700 hover:underline transition-colors">
              {t("viewTechniques")}
            </Link>
          </div>
        </div>
      ...
      <CardContent>
        <div className="flex gap-1.5 mt-1">
          {trend.map(({ date, pct }) => {
            ...
            const dotColor = pct === 100 ? "bg-teal-500" : pct > 0 ? "bg-amber-400" : "bg-slate-200"
            const label = pct === 100 ? t("techniquesDotDone") : pct > 0 ? t("techniquesDotPartial") : t("techniquesDotNone")
            return (
              <div
                key={date}
