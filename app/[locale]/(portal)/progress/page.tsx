...
export default async function ProgressPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const [assessments, recentCheckIns, activeAssignments, techLogs] = await Promise.all([
    ...
    db.query.checkIns.findMany({
      where: (table, { and }) => and(eq(table.userId, userId), gte(table.createdAt, ninetyDaysAgo)),
  ...
  const delta =
    first && latest && assessments.length > 1
  ...
  const currentStreak =
    breakIdx === -1 ? techniqueTrend.filter((d) => d.pct === 100).length : breakIdx;
          ...
          <CardContent>
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="flex flex-col gap-0.5">
                ...
                <span className="text-xl font-bold text-slate-900">
                  {currentStreak}
                  <span className="text-xs font-normal text-slate-400"> {t("days")}</span>
                </span>
