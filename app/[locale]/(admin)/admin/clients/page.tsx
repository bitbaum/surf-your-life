...
import {
  fetchCadenceMap,
  fetchEnergyTrendMap,
  type EnergyTrendEntry,
} from "@/lib/db/check-in-cadence";
...
export default async function ClientsPage({
  ...
  const {
    ...
    practitioner: practitionerParam,
  } = await searchParams;
  ...
  const searchFilter = q?.trim()
    ? or(ilike(users.name, `%${q.trim()}%`), ilike(users.email, `%${q.trim()}%`))
    : undefined;
  ...
  if (clientIds.length > 0) {
    ...
    for (const r of alertCountRows)
      alertCountMap.set(r.clientId, { count: r.alertCount, hasHigh: r.hasHigh });
