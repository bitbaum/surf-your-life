...
export async function ClientProfileCard({ profile }: ClientProfileCardProps) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>{t("detail.profileCard")}</CardTitle>
      </CardHeader>
      <CardContent className="flex flex-col gap-3 text-sm">
        {profile ? (
          <>
            <Row
              label={t("detail.mainConcern")}
              value={
                profile.mainConcern
                  ? tConcerns(profile.mainConcern as Parameters<typeof tConcerns>[0])
                  : undefined
              }
            />
            ...
            <Row
              label={t("detail.workHoursPerWeek")}
              value={
                profile.workHoursPerWeek
                  ? t("detail.hoursPerWeek", { n: profile.workHoursPerWeek })
                  : undefined
              }
            />
            ...
            <Row
              label={t("detail.previousTherapy")}
              value={profile.previousTherapy ? t("detail.yes") : t("detail.no")}
            />
            <Row
              label={t("detail.sleepQuality")}
              value={profile.sleepQuality ? `${profile.sleepQuality}/10` : undefined}
            />
            ...
            <Row
              label={t("detail.stressLevel")}
              value={profile.stressLevel ? `${profile.stressLevel}/10` : undefined}
            />
            <Row
              label={t("detail.socialSupport")}
              value={profile.socialSupport ? `${profile.socialSupport}/10` : undefined}
            />
