      ...
      {rows.length === 0 ? (
        <Card>
          <CardContent className="py-12">
            <EmptyState message={t("empty")} action={<TechniqueCreateButton />} />
          </CardContent>
        </Card>
      ) : (
                      ...
                      >
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 flex-wrap">
                            <p
                              className={`text-sm font-medium ${technique.isActive ? "text-slate-900" : "text-slate-400 line-through"}`}
                            >
                              {technique.name}
                            </p>
                            <span
                              className={`text-xs px-2 py-0.5 rounded-full border ${DIFFICULTY_BADGE_CLASSES[technique.difficulty] ?? DIFFICULTY_BADGE_CLASSES.easy}`}
                            >
