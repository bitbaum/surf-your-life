...
import { formatDate, computeTotalPages, parsePagination, daysSince } from "@/lib/utils";
import {
  PAGINATION_DEFAULT,
  BOOKING_STATUS_BADGE_VARIANT,
  BOOKING_AGE_URGENT_DAYS,
} from "@/lib/constants";
...
export default async function AdminBookingsPage({
  ...
  const activeFilter = (STATUS_FILTERS.find((f) => f.value === statusParam)?.value ?? "all") as
    BookingStatus | "all";
      ...
      <Card>
        ...
        <CardContent>
              ...
              <tbody>
                {allBookings.map((booking) => (
                  ...
                  >
                    <td className="py-3">
                      <Link
                        href={`/admin/clients/${booking.user.id}`}
                    ...
                    <td className="py-3">
                      <Badge
                        label={
                          STATUS_FILTERS.find((f) => f.value === booking.status)?.label ??
                          booking.status
                        }
                    ...
                    <td className="py-3 text-slate-400 text-xs">
                      ...
                      {booking.status === "pending" &&
                        (() => {
                          const days = daysSince(booking.createdAt);
                          return days > 0 ? (
                            <span
                              className={`ml-1.5 px-1.5 py-0.5 rounded-full text-[10px] font-semibold leading-none ${days >= BOOKING_AGE_URGENT_DAYS ? "bg-red-50 text-red-600" : "bg-amber-50 text-amber-600"}`}
                            >
          ...
          <Pagination
            ...
            pageLink={(p) =>
              activeFilter === "all"
                ? `/admin/bookings?page=${p}`
                : `/admin/bookings?status=${activeFilter}&page=${p}`
            }
          />
        </CardContent>
      </Card>
