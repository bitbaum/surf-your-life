...
export async function GET(req: Request) {
  ...
  if (eligibleClients.length === 0)
    return NextResponse.json({ success: true, sent: 0, skipped: allClients.length });
  ...
  for (const client of eligibleClients) {
    ...
    const topWin =
      rawJournal && rawJournal.length > CLINICAL_TEXT_EXCERPT_MAX
        ? rawJournal.slice(0, CLINICAL_TEXT_EXCERPT_MAX - 3) + "…"
        : rawJournal;
