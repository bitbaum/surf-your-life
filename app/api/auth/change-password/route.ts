...
import {
  ...
  BCRYPT_SALT_ROUNDS,
  PASSWORD_MIN_LENGTH,
  API_ERR_NO_PASSWORD_AUTH,
  API_ERR_WRONG_PASSWORD,
} from "@/lib/constants";
...
export async function POST(req: Request) {
  ...
  if (!user?.password) {
    return NextResponse.json({ success: false, error: API_ERR_NO_PASSWORD_AUTH }, { status: 400 });
  }

  const valid = await bcrypt.compare(result.data.currentPassword, user.password);
  if (!valid) {
    return NextResponse.json({ success: false, error: API_ERR_WRONG_PASSWORD }, { status: 400 });
  }
