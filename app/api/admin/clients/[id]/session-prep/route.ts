...
import {
  SESSION_PREP_CHECKIN_LIMIT,
  SESSION_PREP_ALERTS_LIMIT,
...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [
    ...
    db.query.techniqueAssignments.findMany({
      where: and(
        eq(techniqueAssignments.clientId, clientId),
    ...
    db.query.programEnrollments.findFirst({
      where: and(
        eq(programEnrollments.clientId, clientId),
  ...
  const avgTechAdherence =
    clientAssignments.length > 0 && dailyTrend.length > 0
  ...
  const techniqueNames = clientAssignments
    .map((a) => a.technique?.name)
...
Write in a professional, clinical tone. Be specific and actionable.`,
  ...
  const prevAvgEnergy =
    recentCheckIns.length > 1
      ? roundOne(
          recentCheckIns
            .slice(1, SESSION_PREP_ENERGY_AVG_WINDOW + 1)
            .reduce((s, ci) => s + ci.energyLevel, 0) /
            Math.min(SESSION_PREP_ENERGY_AVG_WINDOW, recentCheckIns.length - 1),
        )
      : null;
  const energyDirection: "up" | "down" | "stable" =
    avgEnergy == null || prevAvgEnergy == null
      ? "stable"
      : avgEnergy > prevAvgEnergy + 0.5
        ? "up"
        : avgEnergy < prevAvgEnergy - 0.5
          ? "down"
          : "stable";
...
function buildClinicalContext(
  client: {
    name: string | null;
    profile?: { mainConcern?: string | null; goals?: string | null } | null;
  },
  ...
  if (medications.length > 0) {
    const medList = medications
      .map((m) => [m.medicationName, m.dose, m.frequency].filter(Boolean).join(" "))
      .join("; ");
  ...
  if (latestAssessment) {
    ...
    if (latestAssessment.notes)
      lines.push(
        `  Assessment notes: ${latestAssessment.notes.slice(0, CLINICAL_TEXT_EXCERPT_MAX)}`,
      );
  }
  ...
  if (activeEnrollment?.startDate) {
    ...
    const currentPhase =
      phases?.filter((p) => p.week <= weekNum)?.sort((a, b) => b.week - a.week)[0] ?? null;
  ...
  if (recentCheckIns.length > 0) {
    ...
    for (const ci of recentCheckIns) {
      ...
      const sleep =
        ci.sleepHours != null
          ? ` | sleep ${ci.sleepHours}h${ci.sleepQuality != null ? ` (quality ${ci.sleepQuality}/5)` : ""}`
          : "";
      ...
      lines.push(
        `  ${date}: mood=${ci.mood}, energy=${ci.energyLevel}/10${sleep}${activity}${fatigue}${brainFog}${pain}${stress}${pem}${ortho}`,
      );
      if (ci.journalEntry)
        lines.push(`    → "${ci.journalEntry.slice(0, CLINICAL_TEXT_EXCERPT_MAX)}"`);
    }
  }
...
function buildRuleBasedSummary(
  ...
  const avgEnergy = Math.round(
    recentCheckIns
      .slice(0, SESSION_PREP_ENERGY_AVG_WINDOW)
      .reduce((s, ci) => s + ci.energyLevel, 0) /
      Math.min(SESSION_PREP_ENERGY_AVG_WINDOW, recentCheckIns.length),
  );
